...
node {
  ...
  stage('Upload') {
		...
		rsync --progress -a --include "statix_$DEVICE-*-$STATIX_BUILD_TYPE.zip" --exclude "*" $OUT_DIR_COMMON_BASE/stx-caf/target/product/$DEVICE/ anayw2001@storage.osdn.net:/storage/groups/s/st/statixos/$DEVICE/9/
		'''
